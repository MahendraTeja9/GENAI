...
async def register_user(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    # Get the default company (first active company) for auto-assignment
    default_company = db.query(Company).filter(Company.is_active == True).first()
    if not default_company:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="No company available for user assignment"
        )
    
    # Create new user with auto-assigned company
    ...
    db_user = User(
        ...
        company_id=default_company.id  # Auto-assign to default company
    )
